...
/**
 * Ink analyzer: analyze ink coverage of a document. For some reason, the API crashed when trying to
 * analyze a document on more than one page. To prevent the crash, each page of the document is
 * analyzed separately.
 *
...
public class InkAnalyzer extends AbstractRemoteAnalyzer implements Analyzer {
     ...
     *
     * @param page Single page document
    ...
    private InkAnalysisItem analyzeSinglePage(Document page)
        ...
        String[] gsArgs = {
            ...
            "-dQUIET",
            "-sDEVICE=inkcov",
            "-sOutputFile=" + diskStore.addFile(outputDiskStoreKey),
            "-f",
    ...
    @Override
    public List<AnalysisItem> run(Document document)
            throws IOException, AnalyzerException, DocumentException {
