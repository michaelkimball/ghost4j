...
/**
 * Safe appender : appends a document to another one. Document types can be mixed (example: append
 * PDF to PS). The modified document returned by the modify method is always the same as the source
 * document provided. The parameter APPEND_DOCUMENT (the document to append to the source one) must
 * be passed to the modify method. Important : this appender is safe even for PostScript document
 * generated by diffrent softwares.
 *
...
public class SafeAppenderModifier extends AbstractRemoteModifier {
    ...
    public SafeAppenderModifier() {
        ...
        supportedDocumentClasses = new Class[] {PSDocument.class, PDFDocument.class};
    }
