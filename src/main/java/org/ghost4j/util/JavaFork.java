...
/**
 * This class allows launching another JVM from the current JVM. It takes the same classpath as the
 * parent JVM.
 *
...
public class JavaFork implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            if (redirectStreams) {
                ...
                // standard stream
                StreamGobbler outputStreamGobbler =
    ...
    private String getCurrentClasspath() {
        ...
        if (classLoader instanceof URLClassLoader) {
            ...
            for (int i = 0; i < urls.length; i++) {
                ...
                try {
                    ...
                    cpBuilder.append(s);
                    if (i < urls.length - 1) cpBuilder.append(PATH_SEPARATOR);
            ...
            try {
                ...
                while (urls2.hasMoreElements()) {
                    ...
                    if (s.startsWith("vfs:")) {
                        if (s.contains(".jar")) {
                            URLConnection conn =
                                    new URL(URLDecoder.decode(s, "UTF-8")).openConnection();
                            Object vf = conn.getContent();
                            // Use reflection to call getPhysicalFile() method
                            // org.jboss.vfs.VirtualFile
                            // This eliminates the dependency on JBoss specific jar files
                            Method getPhysicalFile =
                                    vf.getClass().getDeclaredMethod("getPhysicalFile");
                            ...
                            int idxSlashBeforeJar =
                                    jarVFSPath.substring(0, idxJarExt).lastIndexOf(FILE_SEPARATOR);

                            String jarFileName2 =
                                    jarVFSPath.substring(idxSlashBeforeJar + 1, idxJarExt + 4);
        ...
        // else: unknown classloader — leave cpBuilder empty so the java.class.path fallback below
        // is used
