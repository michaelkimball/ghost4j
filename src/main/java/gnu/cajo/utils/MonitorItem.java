...
/**
 * This class is used to instrument an object for invocation logging purposes. It is intended as a
 * replacement for standard RMI logging, in that this logger is aware of the Invoke package
 * methodology, and can decode it properly. Specifically, it will gather information about the
 * calling client, the method called, the inbound and outbound data. It will also record the
 * approximate time between client invocations, the time used to service the invocation, and the
 * approximate percentage of free memory available at the completion of the operation. Subclassing
 * of MonitorItem is allowed; primarily to create self-monitoring classes.
 *
 * <p><i>Note:</i> monitoring an object can be expensive in runtime efficiency. It is best used for
 * debug and performance analysis, during development, or in production, for objects that would not
...
public class MonitorItem implements Invoke {
    ...
    /**
     * This flag can be used to selectively enable and disable monitoring on a class-wide level. By
     * default it is set to false, when true, no output to any logstream will take place.
     */
     ...
     * By default it is set to false, when true, no output to the logstream will take place.
     */
    public boolean LOCALOFF;

    /**
     * The object being monitored. It is declared as public to allow the reference of the
     * MontorItem, and its wrapped object, from a single instance of MonitorItem.
     */
    public final Object item;

    /**
     * This creates the monitor object, to instrument the target object's use. The the logging
     * information will be sent to System.out automatically.
     *
    ...
    /**
     * This creates the monitor object, to instrument the target object's use. The the logging
     * information is passed to the OutputStream, where it can be logged to a file, a socket, or
     * simply sent to the console (System.out). The logged data is in text format.
     *
     * @param item The object to receive the client invocation.
    ...
    /**
     * This creates the monitor object, to instrument the target object's use. The the logging
     * information is passed to an ObjectOutputStream. <i>Note:</i> this type of monitoring provides
     * both the greatest detail, and can be most easily manipulated programmatically. However, it is
     * even </i>more</i> expensive than text logging. The log file can become <i>extremely</i>
     * large, if the objects passed in or out are complex, or if the object is called frequently.
     * Therefore, it is <u>highly</u> recommended to implement the ObjectOutputStream on top of a
     * GZipOutputStream. <i><u>Note</u>:</i> to preserve remote object codebase annotation, all of
     * the objects associated with the particular method invocation log are stored, in order, in an
     * Object array, contained within a java.rmi.MarshalledObject.
     *
     * @param item The object to receive the client invocation.
    ...
    /**
     * This method logs the incoming calls, passing the caller's data to the internal item. It
     ...
     *   <li>The free memory percentage, following the invocation
     * </ul>
     *
     * If the write operation to the log file results in an exception, the stack trace of will be
     * printed to System.err.
     *
     * <p><i><u>Note</u>:</i> Logging may be activated and deactivated administratively as needed on
     * both an instance-wide basis via the field LOCALOFF, and on a class-wide basis via the static
     * field CLASSOFF.
     *
     ...
     * @throws NoSuchMethodException If the method/agruments signature cannot be matched to the
     *     internal object's public method interface.
    ...
    public Object invoke(String method, Object args) throws Exception {
        ...
        Object result = null;
        try {
            return result = Remote.invoke(item, method, args);
        } catch (Exception x) {
            result = x;
            throw x;
        } finally {
            ...
            int freeMemory = (int) ((rt.freeMemory() * 100) / rt.totalMemory());
